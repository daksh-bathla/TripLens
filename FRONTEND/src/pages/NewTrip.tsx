  ...
  return (
    ...
    >
      <header className="mb-10">
        ...
        <h1 className="text-3xl font-bold tracking-tight text-white">Plan Client Trip</h1>

        <div className="flex gap-2 mt-6">
          {STEPS.map((s, i) => (
            <div key={s} className="flex-1">
              <div className={`h-1 rounded-full transition-all duration-300 ${i <= step ? 'bg-primary' : 'bg-white/15'}`} />
      ...
      <div className="bg-card border border-white/10 rounded-2xl p-8">
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                {VIBES.map(v => (
                  ...
                  >
                    <h4 className={`font-bold mb-1 ${formData.style === v.id ? 'text-primary' : 'text-white'}`}>{v.label}</h4>
        ...
        <div className="mt-8 pt-6 border-t border-white/10 flex justify-between items-center">
          <button
            ...
            className={`text-sm font-bold text-slate-400 py-2.5 px-5 rounded-xl hover:bg-white/10 transition-all ${step === 0 ? 'invisible' : ''}`}
          >
            Back
          </button>
