  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-8">
        <Link to="/" className="inline-flex items-center gap-1.5 text-slate-400 hover:text-white transition-colors text-sm font-semibold group">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-6">
          <div className="bg-card border border-white/10 rounded-2xl p-8">
            ...
            <div className="flex flex-wrap gap-3">
              <span className="flex items-center gap-1.5 text-sm font-semibold text-slate-200 bg-white/10 px-3 py-1.5 rounded-lg">
            ...
            {trip.itinerary ? (
              <div className="space-y-6">
                {trip.itinerary.split('\n\n').filter(Boolean).map((day: string, idx: number) => {
                  ...
                  return (
                    <div key={idx} className="border-l-2 border-white/15 pl-4">
                      <h3 className="font-bold text-white mb-2">{title || `Day ${idx + 1}`}</h3>
                      <div className="space-y-1.5">
                        {points.filter(Boolean).map((p, pidx) => (
                          <p key={pidx} className="text-sm text-slate-300">{p.replace(/^[-•] ?/, '')}</p>
                        ))}
                      </div>
                    </div>
                  );
        ...
        <div className="space-y-6">
          <div className="bg-card border border-white/10 rounded-2xl p-6">
            ...
            <p className="text-3xl font-bold text-white mb-1">₹{trip.budget.toLocaleString()}</p>
